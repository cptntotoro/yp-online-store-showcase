...
class OrderRepositoryTest {
    ...
    @Test
    void findByOrderUuid_shouldReturnOrderWhenExists() {
        Mono<OrderDao> testFlow = createTestUser()
                ...
                .flatMap(savedOrder -> orderRepository.findByOrderUuid(savedOrder.getUuid()));

        StepVerifier.create(testFlow)
                .expectNextMatches(order ->
                        order.getStatus() == OrderStatus.CREATED &&
                                order.getTotalPrice().compareTo(new BigDecimal("100.00")) == 0)
                .verifyComplete();
    }
