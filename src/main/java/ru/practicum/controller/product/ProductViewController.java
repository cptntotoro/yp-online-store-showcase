...
public class ProductViewController {
    ...
    public Mono<String> showProductList(
        ...
        Mono<Page<Product>> pageMono =
                (search != null && !search.isEmpty()) ? productService.search(search, pageable) :
                        (sort != null && !sort.isEmpty()) ? productService.getSorted(sort, pageable) :
                                productService.getAll(pageable);

        return pageMono
                .map(productPage -> productPage.map(productMapper::productToProductOutDto))
                .doOnNext(dtoPage -> model.addAttribute("products", dtoPage))
                .thenReturn("product/catalog");
    }
    ...
    public Mono<String> addProducts(@ModelAttribute("products") ProductListInDto products) {
        List<Product> productList = products.getProducts().stream()
                .map(productMapper::productInDtoToProduct) // обычный Product
                .toList();
